...
(define-type Exp
  ...
  (if0E [tst : Exp]
        [thn : Exp]
        [els : Exp])
  (boxE [arg : Exp])         ;;Exp variant for box
  (unboxE [arg : Exp])       ;;Exp variant for unbox
  (setboxE [b : Exp]         ;;Exp variant for setbox
           [v : Exp]))
...
#|
  ...
  20  [doBoxK (k : Cont)]
  25  [doUnboxK (k : Cont)]
  23  [setboxSecondK (r : ExprD) 
                     (k : Cont)]
  24   [doSetboxK (v1 : Value)
                   (k : Cont)]
|#
  ...
  (cond
   ...
   [(s-exp-match? `{lambda {SYMBOL} ANY} s)
    (lamE (s-exp->symbol (first (s-exp->list 
                                 (second (s-exp->list s)))))
          (parse (third (s-exp->list s))))]
   [(s-exp-match? `{box ANY} s)                                ;;condition for parsing boxE
    (boxE (parse (second (s-exp->list s))))]
    [(s-exp-match? `{unbox ANY} s)                             ;;condition for parsing unboxE
     (unboxE (parse (second (s-exp->list s))))]
    [(s-exp-match? `{set-box! ANY ANY} s)                      ;;condition for parsing setboxE
...
(module+ test
  ...
  (test (parse `{box 0}) (boxE (numE 0)))                       ;;test-case for parsing boxE
  (test (parse `{unbox b})(unboxE (idE 'b)))                    ;;test-case for parsing unboxE
  (test (parse `{set-box! b 0})(setboxE (idE 'b) (numE 0)))     ;;test-case for parsing setboxE  
...
(define (compile a-fae env)
  (type-case Exp a-fae
    ...
    [(boxE arg-expr)                                                  ;;type-case for compiling boxE
     (code-malloc1 18 (compile arg-expr env))]
    [(unboxE arg-expr)                                                ;;type-case for compiling unboxE
     (code-malloc1 19 (compile arg-expr env))]
    [(setboxE box-expr val-expr)
     (code-malloc2 21 (compile box-expr env)
                   (compile val-expr env))]))
      ...
      (case (ref updated-ptr-reg 0)
        ...
        [(1 3 5) 
         (begin
           ...
           (move! 3)
           (done 3))]
        [(2 4 6 17 23)                                   ;;case added for setboxSecondK(23) 
         (begin
           ;; Etc. 
           (move! 1)
           (move! 2)
           (done 2))]
        [(16 24)                                         ;;case added for doSetboxK(24) 
         (begin
           (move! 2)
           (done 2))]
        [(20 22 25)                                      ;;case added for doBoxK(20) boxV(22) doUnboxK(25) 
         (begin
           (move! 1)
           (done 1))]
        [(7)
  ...
  (if (= 99 (vector-ref from-memory n))
      ...
      (begin
        (case (vector-ref from-memory n)
          [(0 15 20 22 25)                                       ;;case added for doBoxK(20) boxV(22) doUnboxK(25) 
           ;; size 1
           (begin
             (malloc1 (vector-ref from-memory n)
                      (vector-ref from-memory (+ n 1)))
             (vector-set! from-memory (+ n 1) (- ptr-reg 2)))]
          [(2 4 6 16 17 23 24)                                   ;;case added for setboxSecondK(23) and doSetboxK(24) 
           ;; size 2
           (begin
...
(define sto-reg 0)  ; Store                   ;;added register for store
  ...
  (case (code-ref expr-reg 0)
    ...
    [(14) ; if0
     (begin
       ...
       (set! expr-reg (code-ref expr-reg 1))
       (interp))] 
    [(18) ; box                                                  ;;case for interpreting boxD(18)                                           
       (begin
         (set! expr-reg (code-ref expr-reg 1))
         (set! k-reg (malloc1 20 k-reg))
         (interp))]
    [(19) ; unbox                                                ;;case for interpreting unboxD(19)
     (begin  
       (set! expr-reg (code-ref expr-reg 1))
       (set! k-reg (malloc1 25 k-reg))
       (interp))]
    [(21) ;setbox 
     (begin
       (set! k-reg (malloc2 23
                            (code-ref expr-reg 2)
                            k-reg))
       (set! expr-reg (code-ref expr-reg 1))
       (interp))])) 
  ...
  (case (ref k-reg 0) 
    ...
    [(7) ; doIfK
     (begin
       ...
       (interp))]
    [(20) ;doBoxK                                                 ;;case for continuation using doBoxK(20)
     (begin
       (set! v-reg (malloc1 22  v-reg))
       (set! k-reg (ref k-reg 1))
       (continue))]
    [(25) ;doUnboxK                                                ;;case for continuation using doUnboxK(25)
     (begin
       (set! v-reg (ref v-reg 1))
       (set! k-reg (ref k-reg 1))
       (continue))]    
    [(23) ;setboxSecondK                                           ;;case for continuation using setboxSecondK(23)
     (begin      
       (set! expr-reg (ref k-reg 1))
       (set! k-reg (malloc2 24 v-reg (ref k-reg 2)))
       (interp))]
    [(24) ; doSetboxK                                               ;;case for continuation using doSetboxK(24)
     (begin
...
(module+ test
  ...
  (test/exn (compile (parse `x) mt-env)
            "free variable")
;;test cases for box unbox----------------------------------
  (reset!)
  (ntest (interpx (compile (parse `{{lambda {b} {unbox b}}{box 91}}) mt-env)
                  empty-env
                  (init-k))
         91)
  ...
  (reset!)
  (ntest (interpx (compile (parse `{{lambda {b}
                                      {unbox {unbox {unbox {unbox {unbox {unbox {unbox {unbox {unbox {unbox {unbox {unbox {unbox b}}}}}}}}}}}}}}
                                    {box {box {box {box {box {box {box {box {box {box {box {box {box 21}}}}}}}}}}}}}})
                           mt-env)
                  empty-env
                  (init-k))
         21)
  ...
  (reset!)
  (ntest (interpx (compile (parse `{{lambda {b} {+ {unbox b} 4}}{box 91}}) mt-env)
                  empty-env
                  (init-k))
         95)
...
;;test cases for setbox----------------------------------
;;setbox returns the address of the box
(reset!)
  (ntest (interpx (compile
                   (parse `{{lambda {b}
                               {set-box! b 91}}
